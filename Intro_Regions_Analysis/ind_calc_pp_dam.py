...
PP_BED_FILE = "Polyphen_dam_sites.txt"
...
def extract_genotypes(bed_file):
    ...
    for chr in CHROMOSOMES:
        ...
        with open(bed_file, 'r') as bed_in, open(chr_real_bed, 'w') as bed_out:
            for line in bed_in:
                if line.startswith(f"chr{chr}\t") or line.startswith(f"{chr}\t"):
                    bed_out.write(line)
        with open(PP_BED_FILE, 'r') as pp_in, open(chr_pp_bed, 'w') as pp_out:
            for line in pp_in:
                if line.startswith(f"chr{chr}\t") or line.startswith(f"{chr}\t"):
                    pp_out.write(line)
        ...
        sorted_chr_pp_bed = f"sorted_chr{chr}_pp.bed"
        ...
        subprocess.run([
            ...
            "-i", chr_pp_bed,
            "-g", GENOME_FILE
        ], stdout=open(sorted_chr_pp_bed, 'w'))
        ...
        os.remove(chr_pp_bed)
        os.remove(sorted_chr_real_bed)
        os.remove(sorted_chr_pp_bed)
...
real_output_file = "pp_dam_real_regions_genotypes.csv"
...
random_output_file = "pp_dam_random_regions_genotypes.csv"
