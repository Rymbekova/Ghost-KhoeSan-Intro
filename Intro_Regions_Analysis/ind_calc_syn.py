...
SYN_BED_FILE = "synonymous_sites_single.txt"
...
def extract_genotypes(bed_file):
    ...
    for chr in CHROMOSOMES:
        ...
        chr_syn_bed = f"chr{chr}_syn.bed"
        ...
        with open(bed_file, 'r') as bed_in, open(chr_real_bed, 'w') as bed_out:
            for line in bed_in:
                if line.startswith(f"chr{chr}\t") or line.startswith(f"{chr}\t"):
                    bed_out.write(line)
        with open(SYN_BED_FILE, 'r') as syn_in, open(chr_syn_bed, 'w') as syn_out:
            for line in syn_in:
                if line.startswith(f"chr{chr}\t") or line.startswith(f"{chr}\t"):
                    syn_out.write(line)
        ...
        sorted_chr_syn_bed = f"sorted_chr{chr}_syn.bed"
        ...
        subprocess.run([
            ...
            "-i", chr_syn_bed,
            "-g", GENOME_FILE
        ], stdout=open(sorted_chr_syn_bed, 'w'))
        ...
        os.remove(chr_syn_bed)
        os.remove(sorted_chr_real_bed)
        os.remove(sorted_chr_syn_bed)
...
real_output_file = "syn_real_regions_genotypes.csv"
...
random_output_file = "syn_random_regions_genotypes.csv"
