...
MISS_BED_FILE = "missense_sites_single.txt"
...
def extract_genotypes(bed_file):
    ...
    for chr in CHROMOSOMES:
        ...
        chr_miss_bed = f"chr{chr}_miss.bed"
        ...
        with open(bed_file, 'r') as bed_in, open(chr_real_bed, 'w') as bed_out:
            for line in bed_in:
                if line.startswith(f"chr{chr}\t") or line.startswith(f"{chr}\t"):
                    bed_out.write(line)
        with open(MISS_BED_FILE, 'r') as miss_in, open(chr_miss_bed, 'w') as miss_out:
            for line in miss_in:
                if line.startswith(f"chr{chr}\t") or line.startswith(f"{chr}\t"):
                    miss_out.write(line)
        ...
        sorted_chr_miss_bed = f"sorted_chr{chr}_miss.bed"
        ...
        subprocess.run([
            ...
            "-i", chr_miss_bed,
            "-g", GENOME_FILE
        ], stdout=open(sorted_chr_miss_bed, 'w'))
        ...
        # Clean up temporary files
         ...
         os.remove(chr_miss_bed)
         os.remove(sorted_chr_real_bed)
         os.remove(sorted_chr_miss_bed)
...
real_output_file = "miss_real_regions_genotypes.csv"
...
random_output_file = "miss_random_regions_genotypes.csv"
