#!/usr/bin/env python3
"""
Extract SIFT-deleterious variants from VCF files for **multiple**
individuals, compare sums in ‘real’ regions vs. 100 random replicates,
and report the difference.

Usage:
    python extract_sift_batch.py individuals.txt
"""
...
SIFT_BED_FILE = Path("SIFT_del_sites.txt")
...
def run_for_individual(individual: str) -> None:
    ...
    out_csv = RESULTS_DIR / f"{individual}_sift_del_100.csv"
...
def main() -> None:
    if len(sys.argv) != 2:
        print("Usage: python extract_sift_batch.py <INDIVIDUAL_LIST_FILE>")
        sys.exit(1)
