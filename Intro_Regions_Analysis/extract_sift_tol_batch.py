#!/usr/bin/env python3
"""
Extract SIFT-tolerant variants from VCF files for **multiple**
individuals, compare sums in ‘real’ regions vs. 100 random replicates,
and report the difference.

Usage:
    python extract_sift_tol_batch.py individuals.txt
"""
...
SIFT_BED_FILE = Path("SIFT_tol_sites.txt")
...
def extract_genotypes(bed_file: Path, individual: str) -> List[pd.DataFrame]:
    ...
    for chr_ in CHROMOSOMES:
        ...
        prefix       = f"{individual}_chr{chr_}_tol"
        chr_real_bed = Path(f"{prefix}_regions.bed")
        chr_sift_bed = Path(f"{prefix}_sift.bed")
        ...
        with SIFT_BED_FILE.open() as src, chr_sift_bed.open("w") as dst:
            for line in src:
                if line.startswith(f"chr{chr_}\t") or line.startswith(f"{chr_}\t"):
                    dst.write(line)
        ...
        sorted_real = Path(f"sorted_{prefix}_regions.bed")
        sorted_sift = Path(f"sorted_{prefix}_sift.bed")
        _sort(chr_real_bed, sorted_real)
        _sort(chr_sift_bed, sorted_sift)

        subprocess.run(
            ["bedtools", "intersect", "-a", sorted_real, "-b", sorted_sift,
             "-wa", "-wb", "-sorted"],
...
def run_for_individual(individual: str) -> None:
    ...
    out_csv = RESULTS_DIR / f"{individual}_sift_tol_100.csv"
...
def main() -> None:
    if len(sys.argv) != 2:
        print("Usage: python extract_sift_tol_batch.py <INDIVIDUAL_LIST_FILE>")
        sys.exit(1)
