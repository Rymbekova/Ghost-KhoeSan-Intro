...
LOW_BED_FILE = "../African/final_gerp_low_auto.txt"
...
def process_chromosome(chr, bed_file, individual, low_bed_file, vcf_dir, vcf_file_template, genome_file, temp_dir):
    """
    ...
    chr_low_bed = os.path.join(temp_dir, f"chr{CHROMOSOME}_low.bed")
    sorted_chr_real_bed = os.path.join(temp_dir, f"sorted_chr{CHROMOSOME}_regions.bed")
    sorted_chr_low_bed = os.path.join(temp_dir, f"sorted_chr{CHROMOSOME}_low.bed")
    ...
    # Extract chromosome-specific regions from the LOW BED file
    with open(low_bed_file, 'r') as low_in, open(chr_low_bed, 'w') as low_out:
        for line in low_in:
            if line.startswith(f"chr{CHROMOSOME}\t") or line.startswith(f"{CHROMOSOME}\t"):
                low_out.write(line)
    ...
    subprocess.run(
        ["bedtools", "sort", "-i", chr_low_bed, "-g", genome_file],
        stdout=open(sorted_chr_low_bed, 'w'),
        check=True
    )
...
def extract_genotypes_parallel(bed_file, individual, low_bed_file, vcf_dir, vcf_file_template, genome_file, max_workers=8):
    """
...
def main():
    ...
    real_output_file = f"{INDIVIDUAL}_gerp_low_real_regions_genotypes.csv"
    ...
    random_output_file = f"{INDIVIDUAL}_gerp_low_random_regions_genotypes.csv"
