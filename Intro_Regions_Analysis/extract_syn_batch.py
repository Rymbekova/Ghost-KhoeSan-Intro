#!/usr/bin/env python3
"""
Extract syn variants from VCF files for **multiple**
individuals, compare sums in ‘real’ regions vs. 100 random replicates,
and report the difference.

Usage:
    python extract_syn_batch.py individuals.txt
"""
...
BED_FILE = Path("synonymous_sites_single.txt")
...
def extract_genotypes(bed_file: Path, individual: str) -> List[pd.DataFrame]:
    ...
    for chr_ in CHROMOSOMES:
        ...
        prefix       = f"{individual}_chr{chr_}_syn"
...
def run_for_individual(individual: str) -> None:
    ...
    out_csv = RESULTS_DIR / f"{individual}_syn_100.csv"
...
def main() -> None:
    if len(sys.argv) != 2:
        print("Usage: python extract_syn_batch.py <INDIVIDUAL_LIST_FILE>")
        sys.exit(1)
