#!/usr/bin/env python3
"""
Extract GERP-low variants from VCF files for **multiple**
individuals, compare sums in ‘real’ regions vs. 100 random replicates,
and report the difference.

Usage:
    python extract_gerp_low_batch.py individuals.txt
"""
...
SIFT_BED_FILE = Path("../African/final_gerp_low_auto.txt")
...
def extract_genotypes(bed_file: Path, individual: str) -> List[pd.DataFrame]:
    ...
    for chr_ in CHROMOSOMES:
        ...
        prefix       = f"{individual}_chr{chr_}_low"
...
def run_for_individual(individual: str) -> None:
    ...
    out_csv = RESULTS_DIR / f"{individual}_gerp_low_100.csv"
...
def main() -> None:
    if len(sys.argv) != 2:
        print("Usage: python extract_gerp_low_batch.py <INDIVIDUAL_LIST_FILE>")
        sys.exit(1)
