...
while IFS= read -r sample_name; do
	...
	# Merge intervals from the score and tracts BED files
        bedtools merge -i "$sample_name/$sample_name.$chr.ref50.tgt1.filtered.score.results" > "${sample_name}_score_merged.bed"
        bedtools merge -i "$sample_name/$sample_name.$chr.ref50.tgt1.tracts.bed" > "${sample_name}_tracts_merged.bed"

        # Calculate the length of the merged score and tracts intervals
        score_length=$(awk '{sum += ($3 - $2)} END {print sum}' "${sample_name}_score_merged.bed")
        tracts_length=$(awk '{sum += ($3 - $2)} END {print sum}' "${sample_name}_tracts_merged.bed")
        ...
        echo "Proportion for $sample_name (chr$chr): $proportion"

        # Optionally, you can remove temporary merged BED files if needed
         rm "${sample_name}_score_merged.bed"
         rm "${sample_name}_tracts_merged.bed"
